...
async def lifespan(app: FastAPI):
    ...
    # Use the first available date's features for the distance check
...
def search_locations(query: str = Query(..., min_length=1)):
    """
    Search for coastal locations in the Balearic Islands that have
    nearby jellyfish predictions (within 1km).
    """
    if app.state.forecast_data is None:
        raise HTTPException(status_code=503, detail="Forecast not loaded")
